<template>
    <div class="content">
        <div class="goodsEva" v-for="(item,index) in matarailList" :key="index">
            <div class="top">
                <img :src="item.logo" class="goodsImg">
            ...
            <div class="evaContent">
                <textarea placeholder="素材满足您的期待？说说您的使用心得吧。"  v-model="formItem[index].evaluateName" placeholder-style="font-size:13px;color:#9B9B9B;"></textarea>
            </div>
             <div class="pics">
...
import { diyEvaluate,baseEvaluate,getOss} from "@/api/api";
...
export default {
    ...
    methods:{
        ...
        getDiymatarial() {
            ...
            diyEvaluate({orderId}).then(res => {
                if(!res.data.code) {
                    this.matarailList = res.data;
                     this.formItem =  res.data.map(item=>{
                      return {
        ...
        //   let host = upLoadHost();
        //   wx.uploadFile({
        //           url: host + 'api/common/upload', //开发者服务器 url
        //           filePath: tempFilePaths, //要上传文件资源的路径
        //           name: 'file', 
        //           formData:{
        //               'scene': "images",
        ...
        //           success: ret => {
        //             wx.hideLoading();
        //             let imgPath = JSON.parse(ret.data).path;
        //              _this.formItem[index].imgs.push(imgPath);
        //           },
        //           fail: () => {},
        //       });
      },
    ...
    checkForm() {
      ...
      subEvaluate() {
          ...
          baseEvaluate(formItem).then(res => {
              if(!res.data.code) {
                  toast("发布成功");
