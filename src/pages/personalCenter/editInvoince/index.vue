<template>
  <div>
    <div class="tab">
      <div
        v-for="(item,index) in invoinceTypeArray"
        :key="index"
...
<script>
import { editInvoince } from "@/api/api";
...
export default {
  data() {
    return {
      ...
      index: 0,
      invoinceArray: [
        { name: "个人", normalType: 1 },
        { name: "企业", normalType: 2 }
      ],
      invoinceTypeArray: [
        { name: "普通发票(电子)", invoiceType: 1 },
        { name: "增值税专用发票", invoiceType: 2 }
      ],
      title: "", //普通个人发票抬头
      content: "", //普通个人发票内容
  ...
  onLoad() {
    let invoinceInfo = JSON.parse(this.$mp.query.invoinceInfo);
    this.id = invoinceInfo.id;
    this.invoinceInfo = invoinceInfo;
    this.invoiceType = invoinceInfo.invoiceType;
    this.normalType = invoinceInfo.normalType;
    if (invoinceInfo.invoiceType == 2) {
        this.activeIndex = 1
      //增值发票
      this.erterContent = invoinceInfo.content;
      this.enterpriseName = invoinceInfo.enterpriseName;
      this.enterTaxCode = invoinceInfo.taxCode;
      this.enterMobile = invoinceInfo.mobile;
      this.address = invoinceInfo.address;
      this.bankName = invoinceInfo.bankName;
      this.account = invoinceInfo.account;
      this.nickname = invoinceInfo.nickname;
      this.loginAddress = invoinceInfo.loginAddress;
      this.defaultStatus = invoinceInfo.defaultStatus;
    } else {
      this.activeIndex = 0;
      if( invoinceInfo.normalType == 1) {
          this.invoinceName = '个人'
      } else if(invoinceInfo.normalType == 2) {
           this.invoinceName = '企业'
      }
      this.title = invoinceInfo.title;
      this.content = invoinceInfo.content;
      this.taxCode = invoinceInfo.taxCode;
      this.mobile = invoinceInfo.mobile;
      this.defaultStatus = invoinceInfo.defaultStatus;
    }
  ...
  methods: {
    ...
    save() {
      let params = {};
      if (this.invoiceType == 1 && this.normalType == 1) {
        //普通个人
        params.id = this.id;
      ...
      } else if (this.invoiceType == 1 && this.normalType == 2) {
        //普通企业
        params.id = this.id;
        params.normalType = this.normalType;
        params.invoiceType = this.invoiceType;
