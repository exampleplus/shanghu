<template>
  <div class="content">
    ...
    <div class="goods">
      <div class="goodsInfo">
        <div class="goodsContent" v-for="(goods,index) in orderInfo.orderDataList" :key="index" @click="goodsDetail(orderInfo.isDiy,goods.goodsId)">
          <div class="goodsWrapper">
            ...
            <div class="goodsIntro">
              ...
              <div class="goodsMoney">
                <p>
                  <span class="goodsPrice">￥{{goods.orderAmount}}</span>
                  <span class="originalPrice" v-if="!orderInfo.isDiy">￥ {{goods.goodsMarketPrice}}</span>
                   <span class="mataoriginalPrice" v-else>含素材费￥{{orderInfo.materialPrice}}</span>
                  <span class="goodsNum" v-if="!orderInfo.isDiy">x {{goods.goodsNum}}</span>
                </p>
              </div>
            </div>
          </div>
          <div v-if="!orderInfo.isDiy">
            <div
                ...
                >
                <div class="afterSale" v-if="goods.refundStatus == 2">已退货</div>
                <div class="afterSale" v-else @click="applyAftersale(goods,index)">申请售后</div>
    ...
    <div class="goodsTotalWrapper">
      <ul class="cost">
        ...
        <li>
         ...
         <!-- <li v-if="orderInfo.materialPrice">
    ...
    <div class="orderInfo">
      <div class="orderNum" v-if="orderInfo.orderStatus==1 || orderInfo.orderStatus==2 || orderInfo.orderStatus==3 || orderInfo.orderStatus == 4 || orderInfo.orderStatus == 6 || orderInfo.orderStatus == 7 ">
        <p class="order">
          <span>订单编号：</span>
   ...
   <div class="division">
    ...
    <div class="bottom" v-if="orderInfo.orderStatus==1">
      <div @click="_cancelOrder(orderInfo.id)">取消订单</div>
      <div class="confirm" @click="payNow(orderInfo.orderNo)">立即支付</div>
    </div>
    <div class="bottom" v-if="orderInfo.orderStatus==4">
      <div class="confirm" @click="_confirmOrder(orderInfo.id)">确认收货</div>
    </div>
    <div class="bottom" v-if="orderInfo.orderStatus==6" @click="eva(orderInfo)">
      <div class="confirm">立即评价</div>
    </div>
    <div class="bottom" v-if="orderInfo.orderStatus==7 || orderInfo.orderStatus==3">
...
export default {
  ...
  methods: {
    ...
    //申请售后
    applyAftersale(goods,index) {
      if(goods.refundStatus == 1) {
        toast("您的售后申请正在处理，请耐心等待审核结果");
        return;
      } else {
    ...
    confirmCancel() {
      ...
      let type = this.$mp.query.type;  //如果有type 表示从确认订单页过来  取消订单之后直接返回到上一页
