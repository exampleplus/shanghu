  ...
  <div class="content">
            ...
            <div class="goodsIntro">
              ...
              <div>
                ...
                <div class="goodsMoney">
                  <p>
                    <span class="goodsPrice">￥{{orderInfo.orderPrice}}</span>
    ...
    <div class="money">
      <div class="goodsPrice">
        <span>商品金额</span>
        <span class="goodsMoney">￥{{orderInfo.orderPrice}}</span>
      </div>
      ...
      <div class="sheeping">
       ...
       <!-- <div class="sheeping">
        <span>素材费</span>
        <span>￥{{orderInfo.materialPrice}}</span>
      </div> -->
    </div>
    ...
    <div class="isPay">
        ...
        <section class="setAddress">
            <switch @change="switchChange" :checked="isOffline" color="#FF5100"/>
        </section>
    </div>
    <div class="bottom">
      <div class="total">
      ...
      <div class="calculate" @click="calculate" v-if="!isOffline">支付</div>
      <div class="calculate" @click="calculate" v-else>确定</div>
    </div>
  </div>
</template>
<script>
import { groupPayInfo, groupRreight, groupPay } from '@/api/api'
...
export default {
 ...
 computed:{
   //商品信息
    selGoods() {
       ...
       for(let i = 0; i <  this.goodsList.length; i++) {
          this.goodsList[i].goodsList.map(ele => {
            proList.push({id:ele.id,goodsNum:ele.goodsNum,goodsPrice:ele.goodsPrice})
            return ele
          })
        }
      return proList
    ...
    totalPrice() {
      let goods = this.selGoods;
      let Money = 0;
      for(let i = 0; i < goods.length; i++) {
        Money += goods[i].goodsNum * goods[i].goodsPrice
      }
    ...
    totalMoney() {
      ...
      let totalMoney = this.orderInfo.orderPrice;
  ...
  methods: {
    ...
    _getPayInfo() {
      ...
      groupPayInfo({groupDiyId}).then(res => {
        if(!res.data.code) { 
   ...
   //选择发票
    ...
    switchChange(e) {
      this.isOffline = e.mp.detail.value;
    },
    ...
    calculate() {
     ...
     groupPay(params).then(res => {
       if(!res.data.code) {
         let orderId = res.data.orderId;
         if(isOffline) {    //如果是线下支付
            let url = '/pages/order/teamOrder/teamOrderDetail/main?id=' + orderId + '&type=' + 0 + '&from=confirm' + '&payType=isOffline' 
            wx.redirectTo({url});
            return;
         } else {
           wx.requestPayment({
              ...
              success: () => {
                 setTimeout(() => {
                  this.loading = false;
                    wx.reLaunch({
                      url: "/pages/order/paySuccess/main?id=" + orderId + "&orderType=diyOrder"
                    });
                },200)
              },
              fail: () => {
               this.loading = false;
                //  let url = '/pages/order/teamOrder/main'
                //  wx.redirectTo({url})
                 wx.redirectTo({
                  url: '/pages/order/teamOrder/teamOrderDetail/main?id=' + orderId + '&type=' + 0 + '&from=confirm'  
                });
              }
            });
         }
