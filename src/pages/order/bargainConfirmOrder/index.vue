  ...
  <div class="content">
    ...
    <div class="money">
      <div class="goodsPrice">
        <span>商品金额</span>
        <span class="goodsMoney">￥{{totalgoosPrice}}</span>
      </div>
      <div class="sheeping">
        <span>运费</span>
        <span>￥{{shipFree}}</span>
...
<script>
import { bargainOrderInfo, getRreight, confirmBargainOrder } from '@/api/api'
...
export default {
 ...
 computed:{
  ...
  methods: {
    ...
    _getPayInfo() {
      let bargainId = this.$mp.query.bargainId;
      bargainOrderInfo({bargainId}).then(res => {
        wx.showLoading({
          title:"加载中",
          mask:true
        })
   ...
   //选择发票
    ...
    if(this.loading) {
     ...
     confirmBargainOrder(params).then(res => {
       var _this = this;
       if(!res.data.code) {
         ...
         wx.requestPayment({
              ...
              fail: () => {
                _this.loading = false;
                _this.isGetpayInfo = true;
                toast("支付失败");
                 wx.redirectTo({
                  url: "/pages/order/bargainOrder/main"
                });
              }
            });
       } else {
