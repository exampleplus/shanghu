...
export default {
  ...
  onLoad(options) {
    Object.assign(this.$data, this.$options.data());
     this.arrs = decodeURIComponent(this.$mp.query.scene).split(',');
  ...
  onShareAppMessage(res) {
    ...
    let that = this;
    let id = this.arrs[0]
    let goodsId = this.arrs[1];
    let skuId = this.arrs[2];
    return {
      // 强烈推荐+商品标题名称+商品主图。
      title: `强烈推荐${that.bargianInfo.goodsName}`,
      imageUrl: that.bargianInfo.logo,
      path:"/pages/order/twoshareBargainDetail/main?id=" + id+ '&goodsId=' + goodsId + '&skuId=' + skuId
    };
  },
  methods: {
    backHome() {
...
getImage(path) {
    ...
    _helpBargain() {
      ...
      let bargainId = this.arrs[0]; //砍价id
