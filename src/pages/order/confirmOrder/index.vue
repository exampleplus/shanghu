  ...
  <div class="content">
            ...
            <div class="goodsIntro">
              ...
              <div>
                ...
                <div class="goodsMoney">
                  <p>
                    <span class="goodsPrice" v-if="!isbargainOrder">￥{{goods.goodsPrice}}</span>
                     <span class="goodsPrice" v-else>￥{{goods.goodsBargainMarketPrice}}</span>
                    <span class="originalPrice">￥{{goods.goodsMarketPrice}}</span>
    ...
    <div class="money">
      <div class="goodsPrice">
        <span>商品金额</span>
        <span class="goodsMoney">￥{{goodsTotalMoney}}</span>
      </div>
      <div class="sheeping">
        <span>运费</span>
        <span>￥{{shipFree}}</span>
...
<script>
import {getPayInfo,getRreight,orderPay,pay} from '@/api/api'
...
export default {
 ...
 onShow() {
    ...
    this.loading = false;
    this.orderRemarks = '';
    this.isbargainOrder = 0;
    if(this.$mp.query.isCarPay) { //如果从购物者进入该页面
      this.isCarPay = true
   }
    if(this.$mp.query.bargainOrder) {
      this.isbargainOrder = 1;
    }
   this._getPayInfo();
 },
 onUnload() {
 ...
 computed:{
   //商品信息
    selGoods() {
       let proList = [];
       if(this.$mp.query.bargainOrder) {   //如果是砍价订单
         for(let i = 0; i <  this.goodsList.length; i++) {
          this.goodsList[i].goodsList.map(ele => {
            proList.push({id:ele.id,goodsNum:ele.goodsNum,goodsPrice:ele.goodsBargainMarketPrice})
            return ele
          })
        }
       } else {
     ...
     goodsTotalMoney() {
      ...
      if(this.$mp.query.bargainOrder) {  //如果是砍价订单
        for(let i = 0; i < goods.length; i++) {
          totalMoney += goods[i].goodsNum * goods[i].goodsPrice
        }
       return totalMoney;
      } else {
        for(let i = 0; i < goods.length; i++) {
  ...
  methods: {
    ...
    _getPayInfo() {
      let goodsArr = JSON.parse(this.$mp.query.goodsArr);
      getPayInfo(goodsArr).then(res => {
        if(!res.data.code) { 
   ...
   //选择发票
    ...
    calculate() {
     ...
     let orderAmount = this.totalPrice;
     ...
     let orderDataList = JSON.parse(this.$mp.query.goodsArr);
