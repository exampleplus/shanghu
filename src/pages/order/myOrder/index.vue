<template>
  <div class="myOrder">
    <ul class="tabBar">
      <li @click="tab(0)" :class="[{active:activeIndex == 0}]">全部</li>
     ...
     <ul class="orderList">
      <li v-for="(item,index) in orderList" :key="index">
        ...
        <div class="goodsInfo" >
          <div class="goodsContent" @click="goodsDetail(item.id)" v-for="(good,idx) in item.orderDataList" :key="idx">
            <div class="goodsWrapper">
              ...
              <div class="goodsIntro">
                ...
                <div class="goodsMoney">
                  <p>
                    <span class="goodsPrice" v-if="!item.isDiy">￥{{good.orderAmount}}</span>
                    <span class="goodsPrice" v-else>￥{{item.orderAmount}}</span>
                    <span class="originalPrice" v-if="!item.isDiy">￥{{good.goodsMarketPrice}}</span>
                     <span class="mataPrice" v-else>含素材费{{item.materialPrice}}</span>
                    <span class="goodsNum"  v-if="!item.isDiy">x {{good.goodsNum}}</span>
                  </p>
                </div>
              </div>
            </div>
...
<script>
import {getOrderList,checkSheeping,cancelOrder,delOrder,confirmOrder,pay} from '@/api/api';
...
export default {
  ...
  methods:{
    ...
    _getOrderList() {
      ...
      getOrderList({pageIndex,pageSize,orderStatus}).then(res => {
        if(!res.data.code) {
            wx.hideLoading();
